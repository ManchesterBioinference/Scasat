...
outputFolder = '/mnt/mr01-home01/mqbsxsm2/scratch/Buenrostro_pipeline_MACS_Improved_MultiThread/Human_data_analysis/output/'

# inputFolder : Folder name with all the fastq files
intputFolder = '/mnt/single-cell/users/mqbsxsm2/Buenrostro_ATAC-seq_data/Homosapiens_fastq_files'

# Blacklisted sequences
blackListFile = '/mnt/mr01-home01/mqbsxsm2/Packages/Blacklist_regions/consensusBlacklist.bed'

# Setting up the genome folder
#ref_genome = '/home/baker/my-mm10-index-share/Mus_musculus.GRCm38.71.fa'
ref_genome = '/mnt/mr01-home01/mqbsxsm2/Bowtie2_Human_genome/genome'
...
# GWL: We restrict the number of threads python will start (each will num trimmomatic then STAR, ...)
# GWL: Each software app in those threads will start their own (java) threads. So restrict the number.
# GWL: num_python_threads * num_software_threads must equal $NSLOTS (the number of cores reserved in batch)

# GWL: Number of theads python should start
num_python_threads=4
# GWL: Number of threads each software (trimmomatic, STAR, ...) should start
num_software_threads=4

# GWL: Keep a list of thread objects we create - one per input file.
# GWL: But we only allow a small number of python threads to run at any one time.
thread_list = []
...
for read1, read2 in zip(reads1, reads2):
    ...
    # GWL: This will block if limit of running python threads reached.
    # GWL: Only when another thread finishes will we be release to continue.
